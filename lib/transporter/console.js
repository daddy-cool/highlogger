...
class Console extends AbstractTransporter {
  ...
  write (msg, options, callback) {
    ...
    if (options.severity <= 3) {
      message = msgPrefix + this.chalk.red(stringify(msg, false, this.maxMessageSize - (msgPrefix.length + 10)));
    } else if (options.severity === 4) {
      message = msgPrefix + this.chalk.yellow(stringify(msg, false, this.maxMessageSize - (msgPrefix.length + 10)));
    } else {
      message = msgPrefix + stringify(msg, false, this.maxMessageSize - msgPrefix.length);
    }
