...
function filterPrintUsASCII (str, maxLength) {
  ...
  if (typeof str === OBJECT_TYPE.STRING) {
    if (str.length === 0) {
      return str;
    }
...
class Syslog extends Socket {
   ...
   */
  setJson (json) {
    if (typeof json === OBJECT_TYPE.BOOLEAN) {
      this.json = json;
    } else {
      this.json = false;
    }
  ...
  write (message, options, callback) {
    if (typeof options.date === OBJECT_TYPE.UNDEFINED) {
    ...
    if (typeof options.structuredData !== OBJECT_TYPE.STRING) {
