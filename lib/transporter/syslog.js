...
class Syslog extends Socket {
  ...
  constructor (config) {
    ...
    config.method = 'udp4';
    config.messageLength = (config.messageLength || 2048) - MSG_OVERHEAD;
  ...
  write (message, options, callback) {
    ...
    super.write(
      ...
      + SPACE + this.processId + SPACE + (debug ? this.filterMessageId(options.debugKey) : NIL) + SPACE
      + NIL + SPACE + message,
      {},
      callback
    );
