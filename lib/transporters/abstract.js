...
class Abstract {
  ...
  /**
   * @param {object} config
   * @param {boolean} [config.useContext]
   * @returns {Abstract}
   * @throws {Error}
   */
  setUseContext (config) {
    ...
    if (config.hasOwnProperty('useContext')) {
      if (typeof config.useContext !== constants.TYPE_OF.BOOLEAN) {
        throw new Error(error.config.invalidValue('useContext'));
      }
