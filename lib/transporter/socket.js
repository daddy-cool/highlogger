...
class Socket extends AbstractTransporter {
  ...
  write (msg, options, callback) {
    ...
    if (
      ...
      msgBuffer = new Buffer(msgPrefix + stringify(msg, this.json, this.maxMessageSize -  msgPrefix.length));
    } else {
      msgBuffer = new Buffer(stringify(msg, this.json, this.maxMessageSize));
    }
